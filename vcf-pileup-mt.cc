...
main(int argc, char **argv)
{
	...
	for (; args.end() - args.begin() >= 1 && ( *args.begin() )[0] == '-'; args = { args.begin() + 1, args.end() })
		     if (string_view( *args.begin() ).str() == "-C")
		{
			...
			illumina13 = true;
		else if (string_view( *args.begin() ).str() == "-@")
		{
			if (parse_unsigned( num_threads, string_view(*( args.begin() + 1 )) ))
				args = { args.begin() + 1, args.end() };
			else
				goto usage;
		}
...
//			std::printf("##FORMAT=<ID=DPR,Number=1,Type=Integer,Description=\"Read depth in the backward strand\">\n");
	...
	auto pop_job = [&](std::size_t& job, std::string& line) -> bool {
	...
	auto push_out = [&](std::size_t job, const std::string& line) -> void {
	...
	auto try_pop_out = [&](std::string& line) -> bool {
		std::lock_guard<std::mutex> lock(out_lock);

		if (!( out_data.size() > 0 && !out_data.front().empty() ))
			return false;
	...
	auto pop_out = [&](std::string& line) -> bool {
		std::unique_lock<std::mutex> lock(out_lock);

		while (!( out_data.size() > 0 && !out_data.front().empty() ))
			out_cond.wait(lock);
...
//				               cnt[0][0][1] +                cnt[0][1][1] +                cnt[1][0][1] +                cnt[1][1][1]           , // DPR
	...
	auto worker = [&]() {
		...
		while (pop_job(job, line))
		{
			tokenize(fields, line, '\t');

			string_view chrom = fields[0];

			hts_pos_t pos = ( [](string_view str) -> long {
				unsigned long pos;
				if (parse_unsigned(pos, str))
					return long( pos - 1 );
				else
					return -1;
			} )( fields[1] );

			string_view ref = fields[3];
			string_view alt = fields[4];

			push_out(job, process_mut(buffer, chrom, pos, ref, alt, bams));
		}
	};
